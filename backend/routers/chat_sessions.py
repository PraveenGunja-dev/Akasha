...
def _message_payload(message: models.ChatMessage) -> dict:
    ...
    return {
        ...
        "metadata": {
            ...
            "last_synced_at": sources.get("last_synced_at") if isinstance(sources, dict) else None,
            "answer_generated_at": sources.get("answer_generated_at") if isinstance(sources, dict) else None,
            "source_freshness": sources.get("systems", []) if isinstance(sources, dict) else [],
            "evidence": sources.get("evidence", []) if isinstance(sources, dict) else [],
            "provenance": sources if isinstance(sources, dict) else {},
