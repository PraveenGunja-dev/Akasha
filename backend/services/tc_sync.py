...
def get_auth_token():
    try:
        res = requests.post(AUTH_URL, json=CREDENTIALS, verify=False, proxies={"http": None, "https": None})
...
def fetch_data(endpoint: str, token: str):
    ...
    try:
        res = requests.get(f"{BASE_URL}{endpoint}", headers=headers, verify=False, proxies={"http": None, "https": None})
        res.raise_for_status()
        return res.json()
    except Exception as e:
