...
export default function ExecutiveOverview({ dashboardData, briefing, briefingLoading, briefingError }: any) {
  ...
  const costVariance = poDeliveredCr - (totalPOValue / 10000000);
  ...
  const epsPortfolio = useMemo(() => {
    ...
    projects.forEach((p: any) => {
      ...
      if (!map.has(eps)) map.set(eps, { eps, count: 0, capacity: 0, delayed: 0, totalProgress: 0 });
    ...
    return Array.from(map.values())
      .map(d => ({ ...d, avgProgress: d.count > 0 ? d.totalProgress / d.count : 0 }))
  ...
  const originalScatterOptions = {
    backgroundColor: 'transparent',
    textStyle: { fontFamily: 'inherit' },
    tooltip: { trigger: 'item', formatter: (p: any) => `<strong>${p.data[2]}</strong><br/>Progress: ${p.data[0]}%<br/>Capacity: ${p.data[1]} MW` },
    ...
    series: [{
      ...
      data: projects.map((p: any) => ({ ...p, extractedCap: getProjectCapacity(p) })).filter((p:any) => p.extractedCap > 0).map((p: any) => [Math.round((p.p6?.progress || 0) * 100), p.extractedCap, p.p6_project_name || p.project_name])
    }]
  };
  ...
  const queueScatterOptions = {
    backgroundColor: 'transparent',
    textStyle: { fontFamily: 'inherit' },
    tooltip: { trigger: 'item', formatter: (p: any) => `<strong>${p.data[2]}</strong><br/>Progress: ${p.data[0]}%<br/>Capacity: ${p.data[1]} MW<br/>Status: ${p.data[3]}` },
    ...
    series: [{
      name: 'Projects', type: 'scatter',
      symbolSize: (data: any) => Math.max(8, Math.min(data[1] / 8, 45)),
      itemStyle: { 
        color: (params: any) => params.data[3] === 'Delayed' ? '#ef4444' : params.data[0] >= 90 ? '#10b981' : '#3b82f6',
        opacity: 0.8, borderColor: '#ffffff', borderWidth: 1 
      },
      data: listProjects.map((p: any) => ({ ...p, extractedCap: getProjectCapacity(p) })).filter((p:any) => p.extractedCap > 0).map((p: any) => [Math.round((p.p6?.progress || 0) * 100), p.extractedCap, p.p6_project_name || p.project_name, p.p6?.health || 'On Track'])
    }]
  };
  ...
  return (
    <div className="flex flex-col gap-4 w-full pb-8">
      ...
      >
        ...
        <KPICard title="Cost Variance" value={`₹${Math.abs(costVariance).toFixed(1)} Cr`} subtext={costVariance > 0 ? "Over Budget" : costVariance < 0 ? "Under PO Budget" : "On Budget"} icon={DollarSign} color={costVariance > 0 ? "red" : "emerald"} onClick={() => setActiveKpiModal('Cost Variance')} />
        ...
        <KPICard title="Completed Projects" value={progressStages.completed} icon={Package} color="blue" onClick={() => setActiveKpiModal('Completed Projects')} />
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="show" className="bento-card p-6 flex-1 flex flex-col min-h-[450px]">
        ...
        <div className="flex flex-col gap-4">
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="show" className="bento-card p-6 flex-1 overflow-hidden flex flex-col">
            <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center justify-between mb-4 shrink-0">
              <span className="flex items-center gap-2"><MapPin className="w-5 h-5 text-primary"/> EPS Portfolio</span>
            </h3>
            <div className="space-y-1 overflow-y-auto custom-scrollbar pr-2 flex-1">
              {epsPortfolio.map((node, i) => (
                <div key={i} className="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-800 last:border-0 text-sm">
                  ...
                  <div className="text-xs font-bold text-gray-500 bg-gray-50 dark:bg-gray-800/50 px-2.5 py-1 rounded-md shrink-0 border border-gray-200 dark:border-gray-700">{node.count}</div>
                </div>
              ))}
            </div>
          </motion.div>
