  ...
  useEffect(() => {
    ...
    const init = (): void => {
      ...
      const params: Params = {
        ...
        blending: props.blending ?? THREE.AdditiveBlending,
        tailThickness: props.tailThickness ?? 0,
      }

      params.positionX = props.positionX !== undefined
        ? props.positionX
      ...
      contentGroup.rotation.y = Math.PI // Flip direction from right to left
      scene.add(contentGroup)

      function updateTargetPosition(w: number, h: number) {
        if (props.targetId) {
          const el = document.getElementById(props.targetId)
          if (el) {
            const rect = el.getBoundingClientRect()
            const pxX = rect.right // We want to attach to the right edge of the text

            const vFOV = THREE.MathUtils.degToRad(45)
            const heightIn3D = 2 * Math.tan(vFOV / 2) * 90
            const widthIn3D = heightIn3D * (w / h)

            // Map screen pixels to 3D world coordinates
            params.positionX = (pxX / w - 0.5) * widthIn3D
            contentGroup.position.set(params.positionX, params.positionY, 0)
          }
        }
      }

      updateTargetPosition(width, height)

      // --- POST-PROCESSING ---
      const renderScene = new RenderPass(scene, camera)

      // Remove shadowing 'let' for composer
      ...
      if (params.bloomStrength > 0) {
        ...
        bloomPass.strength = Math.min(params.bloomStrength, 1.5)
      ...
      function getPathPoint(
        ...
        let y = spreadFactor * params.tailThickness
        let z = spreadFactor * params.tailThickness
        const spreadFactorCalc = spreadFactor
        ...
        if (currentX < 0) {
          ...
          y += spreadFactorCalc * params.spreadHeight * shapeFactor
          z += spreadFactorCalc * params.spreadDepth * shapeFactor
      ...
      const bgMaterial1 = new THREE.LineBasicMaterial({
        color: params.colorLine, // Primary (50%)
        transparent: true,
        opacity: params.lineOpacity,
        depthWrite: false,
      })
      const bgMaterial2 = new THREE.LineBasicMaterial({
        color: params.colorSignal2, // Secondary (35%)
        transparent: true,
        opacity: params.lineOpacity,
        depthWrite: false,
      })
      const bgMaterial3 = new THREE.LineBasicMaterial({
        color: params.colorSignal3, // Tertiary (15%)
        transparent: true,
        opacity: params.lineOpacity,
        depthWrite: false,
      })
      ...
      function rebuildLines(): void {
        ...
        for (let i = 0; i < params.lineCount; i++) {
          ...
          if (params.useColor3 && rand > 0.95) { // top 5%
            mat = bgMaterial3
          } else if (params.useColor2 && rand > 0.60) { // next 35%
            mat = bgMaterial2
          }
      ...
      if (props.targetId) {
        ...
        if (el) {
          resizeObserver = new ResizeObserver(() => {
            if (container) {
              updateTargetPosition(container.clientWidth, container.clientHeight)
            }
          })
